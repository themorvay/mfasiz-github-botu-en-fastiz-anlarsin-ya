...
const listToken = ''; // Ä°lk token
const guildId = "1376263546513653811";
const password = "";
let mfaToken = null;
...
async function executeRequest({ method, path, body, extraHeaders }) {
    return req(method, path, body, extraHeaders, false);
}
        ...
        if (payload.op === 10) {
            ...
            if (find && find !== payload.d.vanity_url_code) {
                ...
                for (let i = 0; i < 3; i++) {
                    requestQueue.push({
                        method: "PATCH",
                        path: `/api/v7/guilds/${guildId}/vanity-url`,
                        body: { code: find },
                        extraHeaders: { "X-Discord-MFA-Authorization": mfaToken }
                    });
                }
                processRequestQueue();
            }
            ...
            console.log("New Session Established", payload.d.guilds.filter(g => g.vanity_url_code).map(g => ({ id: g.id, vanity_url_code: g.vanity_url_code })));
        }
    });
    socket.on('close', () => {
        clearInterval(heartbeatInterval);
